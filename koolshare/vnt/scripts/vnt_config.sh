...
if [ -z "$vnt_path" ] ; then
   ...
   if [ "${JFFS_AVAIL}" -lt "4096" ];then
      ...
      vnt_path=/koolshare/bin/vnt-cli
      dbus set vnt_path=$vnt_path
   fi
fi
if [ -z "$vnts_path" ] ; then
...
fun_nat_start(){
    if [ "${vnt_enable}"x = "1"x ] || [ "${vnts_enable}"x = "1"x ];then
	    [ ! -L "/koolshare/init.d/S99vnt.sh" ] && ln -sf /koolshare/scripts/vnt_config.sh /koolshare/init.d/S99vnt.sh
    fi
}
# 定时任务
fun_crontab(){
